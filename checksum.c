...
//=====================================================================
// NEW SECTION 1 : GENERIC BIT STUFFING / DESTUFFING
//=====================================================================
void bitStuffing(char data[], char stuffed[])
{
    ...
    for(i = 0; i < len; i++)
    {
        ...
        if(data[i] == '1')
        {
            ...
            if(ones == 5)
            {
                stuffed[k++] = '0'; // 5 dharava 1 vandha 0 insert aagum
                ones = 0;
            }
        }
        else
...
// Bit Destuffing: Extra '0' bit-ah matnum skip panni original payload bit stream-ah yedukkum
void bitDestuffing(char stuffed[], char destuffed[])
{
    ...
    for(i = 0; i < len; i++)
    {
        ...
        if(stuffed[i] == '1')
        {
            ...
            if(ones == 5)
            {
                i++;        // Next irukura stuffed 0 bit-ah skip pannidurom
                ones = 0;
            }
        }
        else
...
//=====================================================================
#define FLAG_BYTE 01111110 // 01111110
#define ESC_BYTE 01111101 // 01111101
...
//=====================================================================
// NEW SECTION 4 : CHECKSUM LOGIC (8-bit, 1's complement, end-around carry)
//=====================================================================
// Add two 8-bit binary strings, handle carry (end-around carry gets added
// back into the sum, exactly like the manual example you worked out)
...
// end-around carry: if a carry is left after the last addition,
// wrap it around and add it back to the result
if(carry==1)
{
int c=1, i2;
...
// Splits data into 8-bit words, adds them all together (with end-around
// carry), then takes the 1's complement -> that is the checksum
...
if(rem!=0) // leftover bits, pad with 0 on the right to make a full byte
{
for(i=0; i<8; i++)
{
if((nwords*8+i) < len)
...
// Receiver side: add all data words AND the checksum together.
// If the final result is all 1's ("11111111") -> data is VALID.
...
addBinary8(sum, checksum); // add the checksum itself
printf("Final Sum         : %s\n", sum);
printf("========================================================\n");
...
char pppStuffed[10500];   // enlarged: input is now full binaryData, not just fileBinary
char pppDestuffed[10500]; // enlarged to match pppStuffed
// Checksum buffers
//char hdlcChecksum[9];
char pppChecksum[9];
//int hdlcValid;
...
//===================================================================
// CHECKSUM ON THE BIT-STUFFED DATA ("After Bit Stuffing" block -> checksum input)
//===================================================================
...
//===================================================================
// PPP FRAME FORMAT / DESTUFFED FRAME FORMAT
//===================================================================
...
// FIX: input ippo binaryData (full physical layer bit stream: file binary +
// ports) - munnadi fileBinary (file content mattum) eduthuthu, adhu thappu.
// Bit stuffing yeppadi binaryData eduthukudho, byte stuffing kum athey
// binaryData than input.
//===================================================================
...
//===================================================================
// CHECKSUM ON THE BYTE-STUFFED PPP DATA ("After Byte Stuffing" block -> checksum input)
//===================================================================
